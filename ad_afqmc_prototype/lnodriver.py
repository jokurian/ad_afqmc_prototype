...
def run_lnoqmc_energy(
    *,
    ...
    Returns:
      (mean_energy, stderr, block_energies, block_weights, mean_ecorr, err_ecorr, block_ecorr)
    """
    ...
    if print_every:
        print(
            ...
            f"{'W':>12s}    {'nodes':>10s}  {'dt[s/bl]':>10s}  {'t[s]':>7s} {'E_corr':>14s}  {'E_corr_err':>10s}"
        )
    ...
    for start in range(0, params.n_blocks, chunk):
        ...
        print(
            ...
            f"{dt_per_block:9.3f}  "
            f"{elapsed:8.1f}"
            f"{float(stats_ecorr['mu']):14.10f}  "
            f"{float(stats_ecorr['se_star']):10.3e}"
        )
    ...
    data_clean_ecorr, _ = reject_outliers(jnp.column_stack((block_ecorr_s, block_w_s)), obs=0)
    ...
    block_ecorr_s = jnp.asarray(data_clean_ecorr[:, 0])
    print("\nFinal blocking analysis:")
    stats = blocking_analysis_ratio(block_e_s, block_w_s, print_q=True)
    mean, err = stats["mu"], stats["se_star"]
    stats_ecorr = blocking_analysis_ratio(block_ecorr_s, block_w_s, print_q=True)
    mean_ecorr, err_ecorr = stats_ecorr["mu"], stats_ecorr["se_star"]
    print(f"Final ecorr blocking analysis:")
    print(f"Mean: {stats_ecorr['mu']}, StdErr: {stats_ecorr['se_star']}")
